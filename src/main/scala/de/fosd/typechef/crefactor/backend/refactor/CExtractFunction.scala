...
object CExtractFunction extends ASTSelection with CRefactor {
    ...
    def getSelectedElements(morpheus: Morpheus, selection: Selection): List[AST] = {
        ...
        println("ExtractFuncSelection: " + cachedSelectedElements)
    ...
    private def hasVarsToDefinedExternal(selection: List[AST], morpheus: Morpheus): Boolean = {
        ...
        if (!idsToDeclare.isEmpty) println("+++ Invalid selection for: " + selection + " with follwoing ids to declare: " + idsToDeclare)
